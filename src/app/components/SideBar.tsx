        ...
        <div className="relative">
            ...
            <div className={`flex flex-col items-center z-50 ${isOpen ? 'w-44' : 'w-16'} transition-all duration-300 text-light-text dark:text-dark-text bg-light-sub-bg dark:bg-dark-sub-bg rounded-r-xl h-dvh fixed`}>
                <button onClick={() => setIsOpen(!isOpen)}
                    ...
                    {isOpen && <button
                        className={`z-10 bg-light-sub-bg dark:bg-dark-sub-bg flex items-center w-10 h-10 rounded-r-full ${isOpen ? 'bg-light-border' : 'bg-light-sub-bg'} text-white transition duration-200 ease-in-out align-middle justify-center`}
                        onClick={() => setIsOpen(!isOpen)}
                    >
                        <ChevronLeft />
                    </button>}
                ...
                <div className="w-full px-2">
                    <div className="flex flex-col items-center w-full mt-3 border-t border-light-border dark:border-dark-border align-middle">
                        <a className="relative group flex items-center w-full h-12 px-3 mt-2 rounded hover:bg-light-border hover:text-gray-300" href="/director/dashboard">
                            <svg className="w-6 h-6 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                            </svg>
                            {!isOpen && <span className=" absolute left-0 hidden bg-light-border group-hover:left-10 h-12 p-4 transition-all duration-300 rounded z-0">Dashboard</span>}
                            {isOpen && <span className="ml-2 text-sm font-medium ">Dashboard</span>}
                        </a>
                        <a className="flex items-center w-full h-12 px-3 mt-2 rounded hover:bg-light-border hover:text-gray-300" href="#">
                            <svg className="w-6 h-6 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
                            {isOpen && <span className="ml-2 text-sm font-medium ">Search</span>}
                        </a>
                        <a className="flex items-center w-full h-12 px-3 mt-2 text-gray-200 bg-light-border rounded" href="#">
                            <svg className="w-6 h-6 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="flex flex-col items-center w-full mt-2 border-t border-light-border dark:border-dark-border">
                        ...
                        <a className="relative flex items-center w-full h-12 px-3 mt-2 rounded hover:bg-light-border hover:text-gray-300" href="#">
                            <svg className="w-6 h-6 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <a className="flex items-center justify-center w-full h-16 mt-auto bg-gray-800 hover:bg-light-border hover:text-gray-300" href="#">
                    <svg className="w-6 h-6 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
